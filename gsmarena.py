...
for line in lines:
 try:
  ...
  try:
    cursor.execute("INSERT INTO `tidphones` (Modelname, Outmodel) VALUES ('"+ eklemodel.strip() +"', '" + outmodelname.strip() + "')")
  ...
  for line in lines:
    ...
    try:
      ...
      cursor.execute("UPDATE `tidphones` SET `"+ ekle.strip() +"` = RTRIM('" + ekle2.strip() + "') Where Modelname = '"+ eklemodel.strip() +"'")
    except: 
        pass
